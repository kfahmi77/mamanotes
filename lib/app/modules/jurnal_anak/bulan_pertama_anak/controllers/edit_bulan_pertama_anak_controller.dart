...
class EditBulanPertamaAnakController extends GetxController {
  final fotoBulanPertamaAnak = Rx<File?>(null);
  String _downloadURL = '';
  XFile? pickedImage;

  void init(BulanPertamaAnakModel bulanPertamaAnak) {
    if (bulanPertamaAnak.imageUrl.isNotEmpty) {
      fotoBulanPertamaAnak.value = File(bulanPertamaAnak.imageUrl);
      _downloadURL =
          bulanPertamaAnak.imageUrl; // Tugaskan nilai URL gambar sebelumnya
    }
  }
  ...
  Future<void> updateImage(String anakId, String bulanPertamaAnakId) async {
    if (fotoBulanPertamaAnak.value == null || !fotoBulanPertamaAnak.value!.existsSync()) {
