...
class EditBerdiriAnakController extends GetxController {
  final fotoGigiAnak = Rx<File?>(null);
  String _downloadURL = '';
  XFile? pickedImage;

  void init(BerdiriAnakModel berdiriAnakModel) {
    if (berdiriAnakModel.imageUrl.isNotEmpty) {
      fotoGigiAnak.value = File(berdiriAnakModel.imageUrl);
      _downloadURL =
          berdiriAnakModel.imageUrl; // Tugaskan nilai URL gambar sebelumnya
    }
  }
  ...
  Future<void> updateImage(String anakId, String berdiriAnakId) async {
    ...
    try {
      ...
      Get.back();
      Get.back();
      Get.snackbar('Berhasil', 'Data  Anak Berdiri berhasil diupdate');
    } catch (e) {
